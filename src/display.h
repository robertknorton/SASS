...
bool InitOverlays()
{
  ...
  gslc_PageAdd(&m_gui,DISP_PG_MAIN,m_asPageElem,MAX_ELEM_PG_MAIN_RAM,m_asPageElemRef,MAX_ELEM_PG_MAIN);

  gslc_FontAdd(&m_gui,FLOW_PERCENT_FONT_TXT,GSLC_FONTREF_PTR,NULL,5);
  gslc_FontAdd(&m_gui,TEMP_DEGREE_FONT_TXT,GSLC_FONTREF_PTR,NULL,5);
...
#define outlineColor      GSLC_COL_GRAY_LT2
  ...
  ref_PLACEHOLDER = gslc_ElemCreateBox(&m_gui,DISP_ELEM_BG_BOX,DISP_PG_MAIN,(gslc_tsRect){0,0,480,320});
  gslc_ElemSetCol(&m_gui,ref_PLACEHOLDER,GSLC_COL_WHITE,bgColor,GSLC_COL_BLACK);
...
//   m_pElemCheckBox1 = ref_PLACEHOLDER; // Save for quick access
  ...
  ref_PLACEHOLDER = gslc_ElemXGaugeCreate(&m_gui,DISP_ELEM_FLOW_PROGRESS,DISP_PG_MAIN,&FLOW_Gauge_obj,(gslc_tsRect){310,10,160,300},0,100,0,GSLC_COL_BLUE,true);
  gslc_ElemSetCol(&m_gui,ref_PLACEHOLDER,outlineColor,GSLC_COL_GRAY,GSLC_COL_BLACK);
  ...
  ref_PLACEHOLDER = gslc_ElemCreateTxt(&m_gui,DISP_ELEM_FLOW_TXT,DISP_PG_MAIN,(gslc_tsRect){365,35,50,10},gui_flow_str,sizeof(gui_flow_str),FLOW_PERCENT_FONT_TXT);
  gslc_ElemSetCol(&m_gui,ref_PLACEHOLDER,outlineColor,GSLC_COL_GRAY,GSLC_COL_BLACK);
  gslc_ElemSetTxtCol(&m_gui,ref_PLACEHOLDER,textColor);
  //gslc_ElemSetFillEn(&m_gui,ref_PLACEHOLDER,false);  // This slows down the program a ton
  gslc_ElemSetTxtAlign(&m_gui,ref_PLACEHOLDER,GSLC_ALIGN_MID_MID);
  ref_DISP_ELEM_FLOW_TXT = ref_PLACEHOLDER; // Save for quick access

  // Water Temp Progress Bar (vertical)
  // - Demonstration of vertical bar with offset zero-pt showing both positive and negative range
  ref_PLACEHOLDER = gslc_ElemXGaugeCreate(&m_gui,DISP_ELEM_TEMP_PROGRESS,DISP_PG_MAIN,&TEMP_Gauge_obj,(gslc_tsRect){10,10,160,300},0,103,0,GSLC_COL_RED,true);
  gslc_ElemSetCol(&m_gui,ref_PLACEHOLDER,outlineColor,GSLC_COL_GRAY,GSLC_COL_BLACK);
  ...
  ref_PLACEHOLDER = gslc_ElemCreateTxt(&m_gui,DISP_ELEM_TEMP_TXT,DISP_PG_MAIN,(gslc_tsRect){60,35,50,10},gui_temperature_str,sizeof(gui_temperature_str),TEMP_DEGREE_FONT_TXT);
  gslc_ElemSetCol(&m_gui,ref_PLACEHOLDER,outlineColor,GSLC_COL_GRAY,GSLC_COL_BLACK);
  gslc_ElemSetTxtCol(&m_gui,ref_PLACEHOLDER,textColor);
  //gslc_ElemSetFillEn(&m_gui,ref_PLACEHOLDER,false);  // This slows down the program a ton
  gslc_ElemSetTxtAlign(&m_gui,ref_PLACEHOLDER,GSLC_ALIGN_MID_MID);
